...
function History(props) {
    ...
    const handelApi = async (e) => {
        console.log("1111111",e.target.innerText);
                        ...
                        props.history.history.map((item, indx) => {
                            return (
                                <div className="history-item" key={indx} onClick={handelApi}>
                                    ...
                                    {/* <p className="method" style={item.payload.method === "POST" ? { backgroundColor: 'green' } : item.payload.method === "DELETE" ? { backgroundColor: 'red' } : item.payload.method === "PUT" ? { backgroundColor: 'yellow' } : {}}>{item.payload.method}</p> */}
